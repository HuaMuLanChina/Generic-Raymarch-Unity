...
Shader "Shadertoy/snailGLSL" { 
    ...
    SubShader {    
        ...
        Pass {    
			...
			uniform float iTime = _Time.y;
			uniform vec4 _MainTex_TexelSize;

			uniform mat4 _CameraInvViewMatrix;
			uniform mat4 _FrustumCornersES;
			uniform vec4 _CameraWS = _WorldSpaceCameraPos;

			uniform vec3 _LightDir;
			uniform mat4 _MatTorus_InvModel;
			...
			void mainImage(out vec4 fragColor,in vec3 ro, in vec3 rd, in vec2 fragCoord)
			{
			...
			void main ()
			{
				...
				vec4 pos = vec4(gl_Vertex.x, gl_Vertex.y, 0.1, 1.0);
			...
			void main ()
			{
				vec3 rd = normalize(ray.xyz);
				vec4 col;
				mainImage(col,_CameraWS.xyz, rd, uv);
				gl_FragColor = col;
				//gl_FragColor = vec4(_CameraWS.x, _CameraWS.y, _CameraWS.z, 1);
				//gl_FragColor = vec4(uv.y, uv.y, uv.y,1);
				//gl_FragColor = vec4(rd.x, rd.y, rd.z, 1);
				//gl_FragColor = vec4(LocalPos.x, LocalPos.y, LocalPos.z, 1);
			}
			#endif
            ENDGLSL     
        }    
